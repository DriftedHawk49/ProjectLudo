...
@Component({
  selector: 'app-online-mini',
  templateUrl: './../../templates/board.html',
  styleUrls: ['./../../templates/mini-board/mboard.scss', './../../templates/commonStyles.scss']
})
export class OnlineMiniComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    socket.on("MULTI_TOKEN_MOVED", (payload) => {
      let np = this.controller.findNewPositionMini(payload.position);
      ...
      if (this.controller.gameboard[posi.x][posi.y].eligible) {
        ...
        if (toke.length == 1) {
          ...
          if (this.controller.isItLocked(posi.x, posi.y)) {
            ...
            np = this.controller.findNewPositionMini(posi);
